...
class ExternalMolecularGenerator:
    """Generate osimertinib-like molecular analogs"""
    ...
    def __init__(self, generator_type: str = "osimertinib_analogs", **kwargs):
        ...
        if self.generator_type == "osimertinib_analogs":
            ...
            # Alternative cores for EGFR inhibitors
            ...
            # Structural fragments extracted from Osimertinib
            ...
            # Generic modifiers for minor changes
            self.generation_2_modifiers = ["F", "Cl", "Br", "CH3", "CF3", "OH", "NH2", "CN", "OCH3", "OCHF2", "SCH3"]
            
            # Linkers for more significant structural changes
            self.generation_3_linkers = ["C(=O)N", "S(=O)2N", "OC(=O)", "c1ccccc1", "CONH", "SO2", "NHCO", "CH2CH2"]
        print("1: ExternalMolecularGenerator initialized.") # Checkpoint 1

    def generate_molecules(self, n_molecules: int = 100,
                             seed_smiles: Optional[str] = None) -> List[str]:
        """Generate novel molecules using osimertinib-based strategy"""
        generated_smiles = []
        
        logger.info(f"Generating {n_molecules} Osimertinib-like analogs focusing on EGFR mutations.")
        
        osimertinib_mol = Chem.MolFromSmiles(self.osimertinib_full_smiles)
        if osimertinib_mol is None:
            logger.error("Could not parse Osimertinib parent SMILES. Cannot generate analogs.")
            return []
        ...
        for i in range(n_molecules):
            ...
            try:
                ...
                modification_strategy = np.random.choice(["core_exchange", "side_chain_mod", "warhead_mod", "add_linker"],
                                                          p=[0.1, 0.5, 0.2, 0.2])
                ...
                if mol_candidate and Chem.AllChem.SanitizeMol(mol_candidate, catchErrors=True) == Chem.SanitizeFlags.SANITIZE_NONE:
                    generated_smiles.append(Chem.MolToSmiles(mol_candidate))
                else:
                    # Fallback to original Osimertinib if generation fails to sanitize
                    logger.warning(f"Generated SMILES '{current_smiles}' failed sanitization. Falling back to original Osimertinib for analog {i}.")
        ...
        print("2: Molecules generated and deduplicated.") # Checkpoint 2
        return valid_candidates

    def _modify_core(self, current_smiles: str) -> str:
        """Replace pyrimidine core with alternative cores"""
        ...
        if "c1nccc(n1)" in current_smiles:
            temp_smiles = current_smiles.replace("c1nccc(n1)", new_core_smiles_fragment)
            mol_candidate = Chem.MolFromSmiles(temp_smiles)
            if mol_candidate and Chem.AllChem.SanitizeMol(mol_candidate, catchErrors=True) == Chem.SanitizeFlags.SANITIZE_NONE:
                print("3: Core modified successfully.") # Checkpoint 3
                return Chem.MolToSmiles(mol_candidate)
            else:
                logger.warning(f"Core modification '{temp_smiles}' failed sanitization. Returning original SMILES.")
                return current_smiles # Fallback
        
        print("4: Core modification not applied or invalid, returning original SMILES.") # Checkpoint 4
    ...
    def _modify_warhead(self, base_smiles: str) -> str:
        ...
        warhead_replacements = {
            "NC(=O)C=C": ["NC(=O)C#C", "NS(=O)(=O)C=C", "NC(=O)CCl"],
        }
        ...
        for original, alternatives in warhead_replacements.items():
            if original in current_smiles:
                ...
                temp_smiles = current_smiles.replace(original, chosen_replacement)
                mol_temp = Chem.MolFromSmiles(temp_smiles)
                if mol_temp and Chem.AllChem.SanitizeMol(mol_temp, catchErrors=True) == Chem.SanitizeFlags.SANITIZE_NONE:
                    print("5: Warhead modified successfully.") # Checkpoint 5
                    return Chem.MolToSmiles(mol_temp)
                else:
                    logger.warning(f"Warhead modification '{temp_smiles}' failed sanitization. Returning original SMILES.")
                    return current_smiles # Fallback
        
        print("6: Warhead modification not applied, returning original SMILES.") # Checkpoint 6
        return current_smiles
    ...
    def _mutate_osimertinib_side_chains(self, base_smiles: str, modifiers: List[str]) -> str:
        ...
        # Specific Osimertinib-based modifications
        osimertinib_modifications = {
            "c3cn(C)c4ccccc34": ["c3cn(CC)c4ccccc34", "c3cn(CCF)c4ccccc34", "c3cnc4ccccc34"],
            "N(C)CCN(C)C": ["N(CC)CCN(C)C", "NCCN(C)C", "N(C)CCCN(C)C"],
            "COc1cc": ["CCOc1cc", "CFc1cc", "Oc1cc"],
        }
        
        modified_flag = False
        for original, alternatives in osimertinib_modifications.items():
            if original in current_smiles and np.random.random() < 0.4:
                chosen_replacement = np.random.choice(alternatives)
                temp_smiles = current_smiles.replace(original, chosen_replacement)
                mol_temp = Chem.MolFromSmiles(temp_smiles)
                if mol_temp and Chem.AllChem.SanitizeMol(mol_temp, catchErrors=True) == Chem.SanitizeFlags.SANITIZE_NONE:
                    ...
                    logger.warning(f"Side chain modification '{temp_smiles}' failed sanitization. Skipping this modification.")
        
        if modified_flag:
            print("7: Side chains mutated successfully.") # Checkpoint 7
        else:
            print("8: Side chain mutation not applied, returning original SMILES.") # Checkpoint 8
        return current_smiles

    def _add_new_linkers_or_motifs(self, base_smiles: str, linkers: List[str]) -> str:
        """Add more significant modifications with new linkers or motifs"""
        ...
        if eligible_atoms_for_extension and np.random.random() < 0.5:
            try:
                ...
                if chosen_linker in ["CH2CH2", "C"]:
                    new_atom = Chem.Atom('C')
                    new_atom_idx = rwmol.AddAtom(new_atom)
                    rwmol.AddBond(target_atom_idx, new_atom_idx, Chem.BondType.SINGLE)
                    Chem.SanitizeMol(rwmol)
                    print("9: New linker/motif added successfully.") # Checkpoint 9
                    return Chem.MolToSmiles(rwmol)
                    
            except Exception as e:
                logger.warning(f"Error adding linker: {e}. Returning original SMILES.")

        print("10: New linker/motif not added, returning original SMILES.") # Checkpoint 10
        return current_smiles
